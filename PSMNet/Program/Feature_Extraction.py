...
class FeatureExtraction(nn.Module):
    def __init__(self):
        ...
        self.layer3 = self.make_res_layer(ResBlock, 128, 3, 1, 1, 2)
        self.layer4 = self.make_res_layer(ResBlock, 128, 3, 1, 1, 4)  # Dilated konvoluce

        # Spatial Pyramid Pooling (SPP)
        self.spp = SPPModule(128, 32)

        # Poslední konvoluce pro zmenšení počtu kanálů
    ...
    def forward(self, x):
        ...
        x = self.layer3(x2)  # Třetí sada
        x4 = self.layer4(x)  # Čtvrtá sada

        x = self.spp(x4, x2)  # SPP modul

        x = self.conv_final(x)  # Poslední konvoluce
        return x  # Výstup pro cost volume
...
class SPPModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.branch1 = nn.Sequential(
            ...
            conv2d(in_channels, out_channels, 1, 1, 0, 1),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x4, x2):
        ...
        out1 = F.interpolate(self.branch1(x4), size=(H, W), mode='bilinear', align_corners=False)
        out2 = F.interpolate(self.branch2(x4), size=(H, W), mode='bilinear', align_corners=False)
        out3 = F.interpolate(self.branch3(x4), size=(H, W), mode='bilinear', align_corners=False)
        out4 = F.interpolate(self.branch4(x4), size=(H, W), mode='bilinear', align_corners=False)

        return torch.cat([x2, x4, out1, out2, out3, out4], dim=1)  # Spojení všech feature map
